...
pred_hca_clusters <- function(ext.dat, num.cores = 1, mode){
  # clusters from hca bone marrow cells
  data.folds <- data_partition("../data/hca-bm-train-test-dat.rds", ext.dat)
...
pred_pd1_clusters <- function(ext.dat, num.cores = 1, mode){
  # clusters from PD1 clinical trial

  data.folds <- data_partition("../data/pbmc-train-test-dat.rds", ext.dat)
  train.feat.combat <- data.folds[['train']]
...
pred_liver_clusters <- function(ext.dat, num.cores = 1, mode){
  # clusters from nature communications paper, healthy liver donors
  print("training using cells from liver")
  data.folds <- data_partition("../data/liver-immune-train-test-dat.rds", ext.dat)
  train.feat.combat <- data.folds[['train']]
...
pred_bm_clusters <- function(ext.dat, num.cores = 1, mode){
  # clusters from JCI paper, healthy bone marrow doners
  print("training using cells from bone marrow (JCI)")
  data.folds <- data_partition("../data/jci-bm-train-test-dat.rds", ext.dat)
  train.feat.combat <- data.folds[['train']]
...
pred_dataset <- function(ext.dat, num.cores = 1, mode){
  ...
  hca.bm.rf <- pred_hca_clusters(ext.dat, num.cores = 1, mode)
  jci.bm.rf <- pred_bm_clusters(ext.dat, num.cores = 1, mode)
  liver.rf <- pred_liver_clusters(ext.dat, num.cores = 1, mode)
  pbmc.rf <- pred_pd1_clusters(ext.dat, num.cores = 1, mode)
