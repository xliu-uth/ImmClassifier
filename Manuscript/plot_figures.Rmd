...
fs7a <- brca5p_dt[original_annotation!='NKT', ][, annot:=ImmC][, Known:= original_annotation][!grepl("^L:T", annot), annot:="Other"][, c('Known', 'annot')] %>% table %>%
           
            plot_heatmap(title = "", palette= "Greens", 
              minshow = 1, size =14, size2=5, minpct=0, measures = c('Recall', 'Precision'),
                        legend.pos = "none",
...
fs7b <- brca5p_dt[original_annotation!='NKT', ][, annot:=SingleR][, Known:= original_annotation][!grepl("T_cell", annot), annot:="Other"][, c('Known', 'annot')] %>% table %>%
           
            plot_heatmap(title = "", palette= "Blues", 
              minshow = 1, size =14, size2=5, minpct=0, measures = c('Recall', 'Precision'),
                        legend.pos = "none",
           ord1 = c(
                'T_cell:CD4+','T_cell:CD4+_central_memory','T_cell:CD4+_effector_memory','T_cell:CD4+_Naive',
               'T_cell:CD8+', 'T_cell:CD8+_Central_memory', 'T_cell:CD8+_effector_memory', 'T_cell:CD8+_effector_memory_RA', 'T_cell:CD8+_naive',
...
# Zoom in trm , separate cd4 and cd8 trm
```{r}

brca5p_subset_dt %>% 
  melt(id.vars = c('Cell', 'UMAP1', 'UMAP2', 'Cluster')) %>%
  mutate(value = factor(value, levels= brca5p_order_celltypes)) %>%
filter(variable == 'SingleR') %>%
  mutate(variable = factor(variable, levels=order_methods)) %>%
  
  ggplot(aes(UMAP1, UMAP2)) + 
         geom_point(aes(color = value), size = 1, pch = 18) +
          theme_minimal() + 
     theme(legend.pos = "bottom", 
          panel.grid.minor = element_blank(), 
          panel.grid.major = element_blank(),
    ...
    scale_color_manual(values = c(colorRampPalette(brewer.pal(12, "Paired"))(13), 'magenta', 'lightgrey', 'black'),
                       guide = guide_legend(ncol = 3, override.aes = list(size=4))) +
    facet_wrap(value~variable, ncol = 5) + xlim(-10, -3) + ylim(0, 6)
...
```{r}
dcast(data.table(perf)[, median(precision), by = c( 'dataset', 'level', 'method')], dataset+level~method)[, .(dataset, level, ImmClassifier-Garnett, ImmClassifier-SingleR)][, .(mean(V3), mean(V4)), by = level]


dcast(data.table(perf)[, median(precision), by = c( 'dataset', 'level', 'method')], dataset+level~method)[, mean(ImmClassifier), by = level]
...
## Figure S7
```{r}
brca5p_lcpm <- readRDS('data_for_figure_plots/testdata/brca.5p.lcpm.rds')
...
f7 <- data.table (brca5p_lcpm[c(mem_cells, trm_cells, naive_cells), c('ITGAE', 'SELL')],
...
# calcualte brca3p overall recall and prec
```{r}

brca3p_immc_recall <- perf %>% filter(dataset == 'brca3p', method == 'ImmClassifier', !celltype %in% c('Dendritic', 'T', 'CD4', 'CD8', 'L', 'M') ) %>% select(recall)  %>% unlist %>% mean()

brca3p_immc_prec <- perf %>% filter(dataset == 'brca3p', method == 'ImmClassifier', !celltype %in% c('Dendritic', 'T', 'CD4', 'CD8', 'L', 'M') ) %>% select(precision)  %>% unlist %>% mean()



brca3p_sr_recall <- perf %>% filter(dataset == 'brca3p', method == 'SingleR', !celltype %in% c('Dendritic', 'T', 'CD4', 'CD8', 'L', 'M') ) %>% select(recall)  %>% unlist %>% mean()

brca3p_sr_prec <- perf %>% filter(dataset == 'brca3p', method == 'SingleR', !celltype %in% c('Dendritic', 'T', 'CD4', 'CD8', 'L', 'M') ) %>% select(precision)  %>% unlist %>% mean()


brca3p_gn_recall <-  perf %>% filter(dataset == 'brca3p', method == 'Garnett', !celltype %in% c('Dendritic', 'T', 'CD4', 'CD8', 'L', 'M') ) %>% select(recall)  %>% unlist %>% mean()


brca3p_gn_prec <- perf %>% filter(dataset == 'brca3p', method == 'Garnett', !celltype %in% c('Dendritic', 'T', 'CD4', 'CD8', 'L', 'M') ) %>% select(precision)  %>% unlist %>% mean()
```
